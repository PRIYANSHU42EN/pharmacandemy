...
export default function SignupPage() {
  ...
  const checkRateLimit = () => {
    ...
    const attemptsStr = localStorage.getItem("signupAttempts");
    ...
    if (recentAttempts.length >= 5) {
      ...
      return `Too many signup attempts. Please try again in ${waitTime} minute${waitTime !== 1 ? 's' : ''}.`;
    }
    return true;
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await signupWithEmail(email, password, name, referral || undefined);
      if (typeof window !== "undefined") localStorage.removeItem("signupAttempts");
