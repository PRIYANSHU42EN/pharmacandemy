...
export async function POST(request: Request) {
  try {
    ...
    const { data, error } = await resend.emails.send({
      from: 'PharmaCademy <nothing.by.14@gmail.com>', // Replace with your verified domain in production
      to: [email],
      subject: 'Welcome to PharmaCademy',
      html: `
        <div style="font-family: sans-serif; padding: 20px; color: #1a1f3c; background-color: #F9F8F7;">
          <h1 style="color: #F7C5D8;">Welcome to PharmaCademy!</h1>
          <p>Hi ${name || 'Student'},</p>
          <p>Thank you for joining PharmaCademy. We're excited to have you on board.</p>
          ...
          <a href="https://pharmacademy.vercel.app/courses" style="display: inline-block; padding: 12px 24px; background-color: #F7C5D8; color: #1a1f3c; text-decoration: none; border-radius: 20px; font-weight: bold; margin-top: 20px;">Start Learning</a>
