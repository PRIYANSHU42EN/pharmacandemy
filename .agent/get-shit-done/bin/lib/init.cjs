...
function buildAgentSkillsBlock(config, agentType, projectRoot) {
  ...
  for (const skillPath of skillPaths) {
    ...
    // Support global: prefix for skills installed at .agent/skills/ (#1992)
    if (skillPath.startsWith('global:')) {
      ...
      if (!fs.existsSync(globalSkillMd)) {
        process.stderr.write(`[agent-skills] WARNING: Global skill not found at ".agent/skills/${skillName}/SKILL.md" — skipping\n`);
        continue;
      }
      ...
      validPaths.push({ ref: `${globalSkillDir}/SKILL.md`, display: `.agent/skills/${skillName}` });
      continue;
    }
